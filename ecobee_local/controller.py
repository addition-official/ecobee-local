...
class EcobeeController:
    ...
    def __init__(self, pair_files=None, app_folder=None, poll_interval=30.0,
                 names=None, debug=False, comfort_warning=True):
        """
        ...
        comfort_warning: if True (default), the first time a comfort method is
               used, print a one-time note to stderr that comfort control
               depends on the thermostat's firmware and may not work on every
               model. Set False to silence it.
        """
        self.app_folder = app_folder or default_app_folder()
        self.poll_interval = poll_interval
        self.debug = debug
        self._comfort_warning = comfort_warning
        self._comfort_warned = False
    ...
    def _warn_comfort_once(self):
        """Print a one-time stderr note about comfort-firmware variability."""
        if self._comfort_warning and not self._comfort_warned:
            self._comfort_warned = True
            print(
                "[ecobee-local] note: comfort control (home/sleep/away) depends "
                "on the thermostat's firmware and does not work correctly on "
                "every model (e.g. the ecobee3 lite mis-reports it). Verify it "
                "behaves on your device. Pass comfort_warning=False to silence "
                "this.",
                file=sys.stderr,
            )
    ...
    def set_comfort_mode(self, label, mode, timeout=12.0):
        ...
        plus a generic hold. Comfort control depends on device firmware and
        does not work correctly on every model (see comfort_warning).
        """
        self._warn_comfort_once()
        key = str(mode).lower()
